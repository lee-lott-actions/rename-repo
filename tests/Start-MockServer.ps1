...
try {
    while ($listener.IsListening) {
        ...
        elseif ($method -eq "PATCH" -and $path -match '^/repos/([^/]+)/([^/]+)$') {
            ...
            $name = $null
            if ($null -ne $bodyObj) { $name = $bodyObj.name }

            # Validate request body: name must be present/non-empty
            if ([string]::IsNullOrEmpty([string]$name)) {
                $statusCode = 422
                $responseJson = @{ message = "Invalid repository name" } | ConvertTo-Json -Compress
            }
            ...
            elseif ($owner -eq 'invalid-owner' -or $repo -eq 'invalid-repo') {
                ...
                $statusCode = 200
                $responseJson = @{ name = $name } | ConvertTo-Json -Compress
            }
        }
        else {
            $statusCode = 404
